...
namespace PubEnt.BLL
{
    ...
    public class ProductCollection:CollectionBase
    {
        ...
        public int NumItemsForShipping  //Number of line items for physical pubs
        {
            get
            {
                int t = 0;
        ...
        public int NumItemsForEmailing  //Number of line items for ONLINE-ONLY pubs
        {
            get
            {
                int t = 0;
                foreach (Product p in this)
                {
                    //if (p.OrderDisplayStatus != "ORDER" && p.OnlineDisplayStatus == "ONLINE")
                    if (!p.IsPhysicalItem)  //NOT a physical item....must be an email-order
                        t += 1;
                }
                return (t);
            }
        }
        public int TotalQtyForShipping  //Total count of physical pubs in cart
        {
            get
            {
                int t = 0;
        ...
        //public int TotalQtyForEmailing  //Number of ONLINE-ONLY items
        //{
        //    ////Unused -- Provided for completeness
        //}
        public double Cost
        {
            get
        ...
        public string PubsForShipping   //Return csv of physical PUBIDs 
        {
            get
            {
                string temp = "";
        ...
        public string PubsForEmailing      //Return csv of ONLINE-ONLY PUBIDs 
        {
            get
            {
                string temp = "";
                foreach (Product item in this)
                {
                    //if (item.OrderDisplayStatus != "ORDER" && item.OnlineDisplayStatus == "ONLINE")
                    if (!item.IsPhysicalItem)      //NOT physical..must be email-order
                        temp += item.PubId.ToString() + ",";
                }
                return (temp);
            }           
        ...
        public string QtysForShipping       //Return csv of QTYs physical pubs
        {
            get
            {
                string temp = "";
