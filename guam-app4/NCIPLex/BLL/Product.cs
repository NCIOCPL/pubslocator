...
namespace NCIPLex.BLL
{
    ...
    public class Product
    {
        ...
        public int NumQtyLimit
        {
            get 
            {
                ///For NCIPLex if the NCIPL limit is greater than the orderlimit, 
                ///then return the orderlimit as the NCIPL limit
                int orderlimit = GlobalUtils.Utils.GetOrderLimit();
                if (orderlimit > _numqtylimit)
                    return _numqtylimit;
                else
                    return orderlimit;
            }
        ...
        {
            return (SQLDataAccess.GetProductbyProductID(productid));
        }
        public static Product GetPubByPubID(int pubid)
        {
            return (SQLDataAccess.GetProductbyPubID(pubid));
        }

        public static ProductCollection GetPubsByCSV(string csv)
        {
            return (SQLDataAccess.GetProductsByCSV(csv));
        }
