...
function fnSetScroll() {
    ...
    window.name = xPos + "_" + ScrollTop;
}


function Navigate(dropdownID, iToPage) {
    document.getElementById(dropdownID).selectedIndex = iToPage - 1;
    __doPostBack(dropdownID, "");
}

function ViewOrderDetail(dropdownID, hidCustID, hidOrderNum, hidPrevPage, CustID, OrderNum, PrevPage) {
    document.getElementById(hidCustID).value = CustID;
    document.getElementById(hidOrderNum).value = OrderNum;
    document.getElementById(hidPrevPage).value = PrevPage;
    __doPostBack(dropdownID, "");
}